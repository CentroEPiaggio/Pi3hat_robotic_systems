...
namespace pi3hat_state_broadcaster
{
    ...
    CallbackReturn Pi3Hat_State_Broadcaster::on_configure(const rclcpp_lifecycle::State &)
    {
        ...
        if(se_prov)
        {
            stt_msg_.sec_enc_pos.resize(joints_.size(),std::nan(""));
            stt_msg_.sec_enc_vel.resize(joints_.size(),std::nan(""));
        }
    ...
    controller_interface::return_type Pi3Hat_State_Broadcaster::update(const rclcpp::Time & time, const rclcpp::Duration &)
    {
        ...
        if(per_pub_ != nullptr)
        {
            ...
            {
                per_msg_.pack_loss[i] = state_interfaces_[i + 1].get_value();
            }
        ...
        for(size_t i = 0; i < sz; i++)
        {
            ...
            stt_msg_.position[i] = state_interfaces_[1 + sz + 4*i ].get_value();
            // RCLCPP_INFO(get_node()->get_logger(),"temp ind is %ld",1 + sz + 4*i + 1);
            stt_msg_.velocity[i] = state_interfaces_[1 + sz + 4*i + 1].get_value();
            // RCLCPP_INFO(get_node()->get_logger(),"temp ind is %ld",1 + sz + 4*i + 2);
            stt_msg_.effort[i] = state_interfaces_[1 + sz + 4*i + 2].get_value();
            // RCLCPP_INFO(get_node()->get_logger(),"temp ind is %ld",1 + sz + 4*i + 3);
            stt_msg_.temperature[i] = state_interfaces_[1 + sz + 4*i + 3].get_value();
        ...
        for(size_t i = 0; i < sz; i++)
        {
            if(se_flag_[i])
            {
                ...
                stt_msg_.sec_enc_pos[i] = state_interfaces_[ 1 + sz*5 + 2*i +1].get_value();
                stt_msg_.sec_enc_vel[i] = state_interfaces_[ 1 + sz*5 + 2*i +2].get_value();
            }
        }
